...
// refer https://bot-man-jl.github.io/articles/2018/Cpp-Struct-Field-Reflection/static_reflection.h
...
ForEachTuple(Tuple&& tuple, Fn&& fn, std::index_sequence<I...>) {
    using Expander = int[];
    (void)Expander{0,
                   ((void)fn(std::get<I>(std::forward<Tuple>(tuple))), 0)...};
}
...
#define DEFINE_STRUCT_FIELD_X(StructField)                                     \
...
inline constexpr void ForEachField(T&& value, Fn&& fn) {
    ...
    detail::ForEachTuple(struct_schema, [&value, &fn](auto&& field_schema) {
        ...
        static_assert(std::tuple_size<FieldSchema>::value >= 2 &&
                          detail::is_field_pointer_v<
                              std::tuple_element_t<0, FieldSchema>>,
                      "FieldSchema tuple should be (&T::field, field_name)");

        fn(value.*(std::get<0>(
                      std::forward<decltype(field_schema)>(field_schema))),
